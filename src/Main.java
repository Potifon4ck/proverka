...
class Calc {
    ...
    private static String opn(String sIn) throws Exception {
        ...
        for (int i = 0; i < sIn.length(); i++) {
            ...
            if (isOp(cIn)) {
                ...
                while ('(' != cTmp) {
                    if (sbStack.length() < 1) {
                        throw new Exception("Проверьте правильность выражения.");
                    }
    ...
    private static double calculate(String sIn) throws Exception {
        ...
        while(st.hasMoreTokens()) {
            try {
                ...
                if (1 == sTmp.length() && isOp(sTmp.charAt(0))) {
                    if (stack.size() < 2) {
                        throw new Exception("Неверное количество данных" + sTmp);
                    }
                    dB = stack.pop();
                    dA = stack.pop();
        ...
        if (stack.size() > 1) {
            throw new Exception("Количество операторов не верное");
        }
